...
class CmsAssetsController < ApplicationController
  unloadable
  before_filter :authorize_edit, :except => [:download, :thumbnail, :timelink]
  before_filter :find_attachment, :only => [:download, :thumbnail, :timelink, :destroy, :show]
  before_filter :find_attachments, :only => [:edit, :update]
