...
class CmsPage < ActiveRecord::Base
  ...
  [:content, :header, :sidebar].each do |name, params|
    src = <<-END_SRC
    def #{name}_parts
  ...
  def copy_from(arg)
    page = arg.is_a?(CmsPage) ? arg : CmsPage.find_by_name(arg)
  ...
  def digest!
    ...
    self.updated_at.to_formatted_s(:number)
  end
