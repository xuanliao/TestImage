...
class CmsSnippet < ActiveRecord::Base
  ...
  def used_in_pages
    CmsPage.where("#{CmsPage.table_name}.content LIKE '%{% render_snippet '?' %}%'", self.name).order(:name)
  end

  def copy_from(arg)
    snippet = arg.is_a?(CmsSnippet) ? arg : CmsSnippet.find_by_id(arg)
  ...
  def expire_cache
    ...
    CmsPage.joins(:layout).where("#{CmsLayout.table_name}.content LIKE '%{% render_snippet '?' %}%'", self.name).map(&:expire_cache)
  end
